...
const char * const BITCOIN_CONF_FILENAME = "xebec.conf";
const char * const BITCOIN_PID_FILENAME = "xebecd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "xebec" is a composite category enabling all Xebec-related debug output
                if(ptrCategory->count(std::string("xebec"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XebecCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XebecCore
    // Mac: ~/Library/Application Support/XebecCore
    // Unix: ~/.xebeccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XebecCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XebecCore";
#else
    // Unix
    return pathRet / ".xebeccore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty xebec.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override xebec.conf
