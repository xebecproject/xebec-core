...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Xebec related functionality
...
/*
    ...
    allow endless transaction that would bloat Xebec and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Xebec specific functionality
